/*
 * Dynamic search boxes with suggestions.
 */

/*
(function($) {

   $.fn.dynSearch = function(options) {
     var defaultOptions = {
     ...
     return this.each(function(x) {
       var inputForm = $(this);
       var suggestions = $("<div />").addClass("suggestion-entries");
       inputForm.attr("autocomplete", "off").addClass("ac-search").after(suggestions);
       ...
       interval = setInterval(function() {
         ...
         // If nothing has changed, we don't do anything
         if (input == oldInput) return;

         oldInput = input;
         var requestUrl = opts.searchUrl + "?format=json&q=" + input;
         var request = $.getJSON(requestUrl, function(data) {
           suggestions.empty();
           data.forEach(function(item) {
             var entry = $("<div />").addClass("suggestion-entry").text(item.title);
             suggestions.append($('<a href="'+item.url+'" />').append(entry));
           });
         });
       }, 400);
     });
